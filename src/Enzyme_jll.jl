...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import Enzyme_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("Enzyme_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "Enzyme_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"Enzyme", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "Enzyme_jll", "src", "Enzyme_jll.jl"))
        @info("Enzyme_ll dev'ed out to /home/vchuravy/src/Enzyme_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("7cc45869-7501-5eee-bdea-0790c847d4ef"))
platforms = [Pkg.Artifacts.unpack_platform(e, "Enzyme", artifacts_toml) for e in artifacts["Enzyme"]]
...
if best_platform === nothing
    @debug("Unable to load Enzyme; unsupported platform $(triplet(platform_key_abi()))")
else
